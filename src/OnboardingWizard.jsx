...
export function OnboardingWizard({ user, session, onComplete, onJoinProject, onSkip, signOut }) {
  ...
  const [inspoFiles, setInspoFiles] = useState([]);     // { file, preview, name }[]
  const [inspoUrlInput, setInspoUrlInput] = useState("");
  const [inspoUrlAdded, setInspoUrlAdded] = useState([]); // string[]
  ...
  // Progress bar (create steps only)
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Seed starter content + log activation in parallel (best-effort)
      ...
      // Fetch invite code for the invite step
  ...
  const doJoin = async (code) => {
    ...
    try {
      ...
      // If App.jsx already handled setProjectId, the wizard unmounts naturally
    } catch {
    ...
    <div className="min-h-screen bg-[#FAF6F0] flex flex-col">
      {progressIdx >= 0 && (
        <div className="w-full h-0.5 bg-black/8">
          <div
            className="h-0.5 bg-slate-800 transition-all duration-500"
      ...
      <div key={step} className={`flex-1 flex flex-col items-center justify-center px-5 py-10 ${animClass}`}>
        <div className="w-full max-w-sm">
          ...
          {step === "loading" && (
            <StepLoading message={loadingMessage} />
          )}
...
function StepWelcome({ firstName, onStart, onHaveCode, signOut }) {
  return (
    <div className="flex flex-col items-center text-center gap-6">
      ...
      <button
        onClick={signOut}
        className="text-xs text-slate-300 hover:text-slate-500 transition-colors mt-1"
      >
        Se déconnecter
      </button>
    </div>
            ...
            <button
              ...
              className={`rounded-xl border py-2.5 px-3 text-xs font-medium text-left transition-all ${
                sel
                  ...
                  : "bg-white text-slate-700 border-black/10 hover:border-slate-400"
              }`}
            >
              {ROOM_LABELS[key]}
            </button>
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PrimaryBtn onClick={onNext}>
        {decoStyle ? "Suivant" : "Continuer sans choisir"}
      </PrimaryBtn>
    </div>
  );
}

function StepInspo({ inspoFiles, inspoUrlAdded, inspoUrlInput, setInspoUrlInput, onFileChange, onRemoveFile, onAddUrl, onRemoveUrl, fileInputRef, onNext, onBack }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <PrimaryBtn onClick={onNext}>
        Créer mon appartement {hasContent ? "→" : ""}
      </PrimaryBtn>
